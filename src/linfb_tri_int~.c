...
static t_class *linfb_tri_int_tilde_class = NULL;
...
void *linfb_tri_int_tilde_new(void) {
  t_linfb_tri_int_tilde *x = (t_linfb_tri_int_tilde *)pd_new(linfb_tri_int_tilde_class);
...
void linfb_tri_int_tilde_free(t_linfb_tri_int_tilde *self) {
  freebytes(self->buffer, 2 * sizeof(t_sample) * self->block_size);
}

t_int *linfb_tri_int_tilde_perform(t_int *w) {
  t_linfb_tri_int_tilde *self = (t_linfb_tri_int_tilde *)(w[1]);
...
void linfb_tri_int_tilde_dsp(t_linfb_tri_int_tilde *self, t_signal **sp) {
  ...
  dsp_add(linfb_tri_int_tilde_perform, 6, self, sp[0]->s_vec, sp[1]->s_vec,
          sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec);
}

void linfb_tri_int_tilde_setup(void) {
   ...
   */
  CLASS_MAINSIGNALIN(linfb_tri_int_tilde_class, t_linfb_tri_int_tilde, x_f);
}

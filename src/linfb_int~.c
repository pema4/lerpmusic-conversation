...
static t_class *linfb_int_tilde_class = NULL;

typedef struct _linfb_int_tilde {
  t_object x_obj;
  t_float x_f;
...
void *linfb_int_tilde_new(void) {
  t_linfb_int_tilde *x = (t_linfb_int_tilde *)pd_new(linfb_int_tilde_class);
...
void linfb_int_tilde_free(t_linfb_int_tilde *self) {
  freebytes(self->buffer, 2 * sizeof(t_sample) * self->block_size);
}

t_int *linfb_int_tilde_perform(t_int *w) {
  t_linfb_int_tilde *self = (t_linfb_int_tilde *)(w[1]);
...
void linfb_int_tilde_dsp(t_linfb_int_tilde *self, t_signal **sp) {
  ...
  dsp_add(linfb_int_tilde_perform, 6, self, sp[0]->s_vec, sp[1]->s_vec,
          sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec);
}

void linfb_int_tilde_setup(void) {
  linfb_int_tilde_class =
      class_new(gensym("linfb_int~"), (t_newmethod)linfb_int_tilde_new,
                (t_method)linfb_int_tilde_free, sizeof(t_linfb_int_tilde),
                CLASS_DEFAULT, NULL);

  class_addmethod(linfb_int_tilde_class, (t_method)linfb_int_tilde_dsp,
                  gensym("dsp"), A_CANT, 0);
  /* if no signal is connected to the first inlet, we can as well
   * connect a number box to it and use it as "signal"
   */
  CLASS_MAINSIGNALIN(linfb_int_tilde_class, t_linfb_int_tilde, x_f);
}
